...
\subsection{View as linear function}

If define
\[
	D(\boldsymbol{x}) =
	\det(\boldsymbol{\alpha_1}, \boldsymbol{\alpha_2}, \dots, \boldsymbol{x}, \dots, \boldsymbol{\alpha_n})
\] 
then $D(\boldsymbol{x})$ is a linear function of $\boldsymbol{x}$, and
\[
	D(\boldsymbol{u}+\boldsymbol{v}) = D(\boldsymbol{u}) + D(\boldsymbol{v})
\] 
\begin{example}
	\[
	\begin{vmatrix}
	...
	Try to find $|\A|$
	 \begin{answer}
		 \[
		 x=\begin{vmatrix}
			 \A & \boldsymbol{\alpha}+0 \\
			 \boldsymbol{\beta}^{T} & c+b-c \\
		 \end{vmatrix}
		 =\begin{vmatrix}
			 \A & \boldsymbol{\alpha} \\
			 \boldsymbol{\beta}^{T} & c \\
...
\] 
We should especially note the 3 order X shape, which not obviously like bigger
ones, but when you recognize it, it will be very easy to calculate.
...
\[
	(c-\lambda)[(a-\lambda)(e-\lambda)-bd]=0
\]
which directly gives one eigenvalue $\lambda_1=c$, and the other two eigenvalues
satisfy
\[
	\begin{aligned}
		\lambda_1+\lambda_2 = a+e \\
		\lambda_1\lambda_2 = ae - bd \\
	\end{aligned}	
\] 
...
\subsection{4 order determinant shortcut}

When calculating 4 order determinant, we can use the following shortcut:
\[
	\newcommand{\nline}{\\ \\ \hline \\}
	\begin{array}{c}
\begin{vmatrix}
...
=
	\begin{array}{cc|c|c}
		\text{sign} & \text{term} & \text{pattern} & \text{shape} \\ 
		\hline \\
		+
&
\begin{vmatrix}
...
\begin{answer}
	\[
	\det = a^2a^2+0+a(-a)+-a(a)-a^2--a(-a)=a^{4}-2a^2
	\] 
\end{answer}
\end{example}
...
\] 
\subsection{Trick to insert Identity matrix in determinant}
...
Moreover, when $\B$ is a single column $\boldsymbol{b}$, it gives a equivalent
condition for  $\A\boldsymbol{x}=\boldsymbol{b}$ having solution:
\begin{equation}
	r(\A , \boldsymbol{b}) = r(\A)
\end{equation}
...
$ \begin{cases}
	...
	ax_1+bx_2=2
\end{cases} $
has solution and $ \begin{vmatrix}
	a & 0 & 1 \\
	1 & a & 1 \\
	1 & 2 & a \\
\end{vmatrix} =4 $,please find the determinant
$
\begin{vmatrix}
	1 & a & 1 \\
	1 & 2 & a \\
...
$
\begin{answer}
Let $\A$ denote the coefficient matrix, and $\overline{\A}$
denote the augmented matrix. The solution existence condition gives
\[
	r(\A) = r(\overline{\A}) \leq 3 < 4
\]
however, $\overline{\A}$ is a 4 by 4 matrix, so it means $|\overline{\A}|=0$, that is
\[
	\begin{vmatrix}
		a & 0 & 1 & 1 \\
		1 & a & 1 & 0 \\
		...
		a & b & 0 & 2 \\
	\end{vmatrix} = 
	-1\cdot \text{(desired determinant)} + 2\cdot 4 = 0
\] 
thus the desired determinant equals 8.
\end{answer}
\end{example}

a direct inference is that when $\A$ is invertible or row full rank,
\[
	r(\A , \B) = r(\A)
\]
...
\]
And it is also easy to calculate powers of rank 1 matrix:
\[
...
\begin{itemize}
	...
	\item $(\A^T)^{*} = (\A^{*})^T$
	\item $(\A^{-1})^{*}=(\A^{*})^{-1}=\dfrac{ \A}{|A|}$ \\
		above two equalities told us that \underline{adjugate, inverse and transpose
		operations are \textbf{commutative}}.
	\item $(\A\B)^{*} = \B^{*}\A^{*}$
	\item $|\A^{*}| = |A|^{n-1}$
	\item $(\A^{*})^{*} = |A|^{n-2}\A$
	\item $\rank(\A^{*})=
...
\begin{example}
	Given an invertible n by n matrix $\A$, satisfying $(\A^{*})^{-1}=(\A^{T})^{*}$ and
	$|\A|<0$, find the determinant of $\A$.
	\begin{answer}[Method 1]
		calculate the determinants of both sides:
		\[
			\begin{aligned}
				|(\A^{*})^{-1}| &= |(\A^{T})^{*}| \\
				\frac{1}{|\A^{*}|} &= |A|^{n-1} \\
	...
	\end{answer}
	\begin{answer}[Method 2]
		since $(\A^{*})^{-1}=(\A^{-1})^{*}$, we have
		\[
			\A^{-1} = \A^{T}
		\] 
		which means $\A$ is orthogonal, thus $|A|=-1$ because $|A|<0$.
	\end{answer}
\end{example}
...
\subsubsection{Factorization to find inverse}

If gives
\[
	\A^2+b\A+c\I=\boldsymbol{O}
\] 
and asking for $(\A+m\I)^{-1}$, we can assume the equation can be factorized as
\[
  (\A+m\I)(\A+p\I)=q\I
\] 
...
A classic example is to find inverse of a matrix $\I-\A$ where $\A$ satisfying $\A^{k}=0$.
\begin{answer}
	since $\I=\I-\A^{k}$
and we have
\[
...
In addition, we have
\[
	(\I+\A)^{-1}=\I-\A+\A^{2}-\A^{3}+\cdots+(-1)^{k-1}\A^{k-1}
\] 
notice that there is no requirement for k to be even or odd, since here 
$A^{k}=(-A)^{k}=0$ make the k odd/even doesn't matter.
...
\begin{example}
If $\boldsymbol{\psi}_1, \boldsymbol{\psi}_2$ are two solutions
...
\begin{itemize}
	...
	\item $\A\sim\B$ implies $\A ^{-1} \sim \B^{-1}, \A^{T} \sim \B^{T}, \A^{*} \sim \B^{*}$\\
		...
		matrix $\A,\A^{T},\A^{*},\I$ and their integer powers(including negative powers
		...
		An useful case is $\A \sim \B \Rightarrow \A + k\I \sim \B + k\I$ and so
		$|\A+k\I| = |\B + k\I|$ \\
		\begin{example}
		If $\A$ is an 3 by 3 matrix, $\boldsymbol{\alpha}$ is a three dimensional vector,
		$\boldsymbol{P}=(\boldsymbol{\alpha}, \A\boldsymbol{\alpha}, \A^{2}\boldsymbol{\alpha})$ is invertible,
		$\B = \boldsymbol{P}^{-1}\A \boldsymbol{P}$, and $\A^{3}\boldsymbol{\alpha} + 2\A^{2} \boldsymbol{\alpha}
		= 3\A \boldsymbol{\alpha} $ try to find $|\A+\I|$
		\begin{answer}
		we have
		\[
		\A\boldsymbol{P}=
		(\A\boldsymbol{\alpha}, \A^{2}\boldsymbol{\alpha}, \A^{3}\boldsymbol{\alpha}) =
		(\A\boldsymbol{\alpha}, \A^2\boldsymbol{\alpha}, 3\A \boldsymbol{\alpha} - 2\A^{2} \boldsymbol{\alpha}) = 
		\boldsymbol{P}
		\begin{bmatrix}
			0 & 0 & 0 \\
			1 & 0 & 3 \\
...
are the eigenvalues of $\A$. Especially, when given $\mathtt{f} (\A)=\boldsymbol{O}$,
then equating $\mathtt{f} (\lambda)=0$ gives us \textbf{\underline{possible}} eigenvalues of $\A$.
\[
	\begin{aligned}
		\lambda \text{ is eigenvalue of } \A \Rightarrow \mathtt{f} (\lambda) = 0 \\
		\mathtt{f} (\lambda) = 0 \not\Rightarrow \lambda \text{ is eigenvalue of } \A \\
	\end{aligned}
\] 
since some roots may not be eigenvalues of $\A$, (e.g. when $\A$ is real
symmetric matrix, the complex roots are not eigenvalues of $\A$.)
...
\label{eq:hasEigenCondAboutAlgoGeoMulti}
\end{equation}
and $\A$ is diagonalizable iff for every eigenvalue $\lambda_i$ the right side of
equation hold as equality, that is also called geometric
multiplicity equals algebraic multiplicity.
 \[
	n - r(\A-\lambda_i \I) = k
\] 
it has another equivalent form about sum of geometric multiplicities
\[
...
\[
 r(\A-k\I) \leq n - 1
\] 
then $k$ is an eigenvalue of $\A$. This can be derived from \eqref{eq:hasEigenCondAboutAlgoGeoMulti},
or directly from the fact that $r(\A-k\I) \leq n - 1 \Rightarrow |\A - k\I| = 0$.

\subsection{General method to find eigenvalue for 3 order matrix}

\subsection{Diagonalization}

If $\A$ is diagonalizable, then there exists an invertible matrix $\boldsymbol{P}$
such that $\A=\boldsymbol{P}\boldsymbol{\Lambda}\boldsymbol{P}^{-1}$,
where $\boldsymbol{\Lambda}$ is a diagonal matrix with diagonal elements
being the eigenvalues of $\A$. Since it is clear that
\[
	r(\A) = r(\boldsymbol{\Lambda})
\] 
we have
\[
	\text{The number of non-zero eigenvalues of } \A = r(\A)
\] 
that's usful when determining eigenvalues multiplicity, especially when only
one eigenvalue is non-zero, then its multiplicity is $r(\A)$.

\begin{example}
An real symmetric matrix $\A$ satisfies $\A^{4}+2\A^{3}+\A^{2}+2\A=\boldsymbol{O}$,
and $r(\A)=r$, then find $|\A+3\I|$.
\begin{answer}
the equation gives $\lambda(\lambda ^2+1)(\lambda +2)=0$, since
$\A$ is real symmetric, the eigenvalues are real, thus the eigenvalues of $\A$ are
$0$ and $-2$. Since $r(\A)=r$, there are $r$ non-zero eigenvalues, thus
the eigenvalue $-2$ has multiplicity $r$, and eigenvalue $0$ has multiplicity $n-r$
, thus
\[	|\A+3\I| = (0+3)^{n-r}(-2+3)^{r} =  3^{n-r} \]
\end{answer}
\end{example}

\subsubsection{ Triangular matrix with all diagonal elements same}
...
\subsection{Eigen for orthogonal matrix}

If $\boldsymbol{Q}$ is an orthogonal matrix, then its eigenvalues have absolute value 1,
that is, they are either 1, -1, or complex numbers on the unit circle since
\[
	\|\boldsymbol{Q}\boldsymbol{v}\|^{2} = (\boldsymbol{Q}\boldsymbol{v})^{T}(\boldsymbol{Q}\boldsymbol{v})
	= \boldsymbol{v}^{T}\boldsymbol{Q}^{T}\boldsymbol{Q}\boldsymbol{v}
	= \boldsymbol{v}^{T}\boldsymbol{v} = \|\boldsymbol{v}\|^{2}
