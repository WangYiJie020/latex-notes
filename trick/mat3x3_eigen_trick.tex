...
\title{Trick to get eigenvalue of Matrix 3$\times$3}
\author{}
\maketitle

\section{Basic Trick}

\subsection{Obvious eigenvalue}

If a 3$\times$3 matrix \( \boldsymbol A \) has linearly dependent rows or columns,
then it has an eigenvalue of 0.

If the sum of the elements in any row or column of \( \boldsymbol A \) is
equal to a constant \( k \), then \( k \) is an eigenvalue of \( \boldsymbol A \).
This is because the vector of all ones \( \boldsymbol v = (1, 1, 1)^T \)
is an eigenvector corresponding to the eigenvalue \( k \)


\section{Trick to expand the determinant}

Calculating the eigenvalues of a 3$\times$3 matrix \( \boldsymbol A \) directly by
...
The characteristic polynomial of a 3$\times$3 matrix \( \boldsymbol A \) is given by:
\[
...
If a 3$\times$3 matrix can be decomposed into a rank 1 matrix, it means that the matrix can be expressed as the outer product of two vectors. Such a matrix has some interesting properties, particularly regarding its eigenvalues.

when a 3$\times$3 matrix \( \boldsymbol A \) can be written as:
\[
\boldsymbol A = \boldsymbol R +\boldsymbol I_3 = \boldsymbol\alpha \boldsymbol\beta^{T} + k\boldsymbol I_3
\] 
where \( \boldsymbol\alpha \) and \( \boldsymbol\beta \) are 3-dimensional vectors, and \( \boldsymbol I_3 \) is the 3$\times$3 identity matrix. We can easily find the eigenvalues of \( \boldsymbol A \) by using the properties of rank 1 matrices.
...
But here is a trick to find \( k \) when the matrix \( \boldsymbol A \) is 3$\times$3.
...
center element directly, but on this situation, $\boldsymbol R$ normally will not
be rank 1 matrix. But if $\boldsymbol{R}$ has 2 rows linearly dependent, which
means $k$ is an eigenvalue of $\boldsymbol{A}$, then we can use this information
to find other eigenvalues by solving the reduced quadratic equation:
\[
	\lambda^2 - (\tr(\boldsymbol A) - k)\lambda + |\boldsymbol A|/k = 0
\]
that is:
\[
\begin{align}
	m &= \frac{\lambda_2+\lambda_3}{2} = \frac{\tr(\boldsymbol A) - k}{2}\\
	p &= \lambda_2 \lambda_3 = \frac{|\boldsymbol A|}{k}\\
	\lambda_{2,3} &= m \pm \sqrt{m^2 - p}\\
\end{align}
\] 
\[
	\boldsymbol{A}=
...
\end{vmatrix}
\] 
has 2 rows linearly dependent, so $k=5$ is indeed an eigenvalue.
Then we can write
\[
	\begin{align}
		|\boldsymbol{A}|&=25 \\
...
gives:
\[
	\lambda_{2,3} = 4 \pm \sqrt{16 - 5} = 4 \pm \sqrt{11}
\] 

\subsection{Not applicable cases}

Not all 3$\times$3 matrices can be decomposed into a rank 1 matrix plus a scalar multiple of the identity matrix. But when $k$ can be found by the above trick, the $\boldsymbol R$ probably has 2 rows linearly dependent, which means $k$ is an eigenvalue of $\boldsymbol A$. But notice that's not always true.
